...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads,  
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, local_enhence=False, abs_emd=True, use_checkpoint=False) -> None:
        """Transformer Block
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, local_enhence=local_enhence)
...
if __name__ == "__main__":
    tsb = Transformer1d(4, 4, 32)
    tsb = tsb.cuda()
    x = torch.rand(16,  4, 200)
    x = x.cuda()
    print(tsb(x).shape)
