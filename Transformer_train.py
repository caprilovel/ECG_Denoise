...
#--------------------------------------#
from local_utils.local_utils import batch_arrythdata_iter, Seq2Seq2, batch_norm_arrythdata_iter
...
def train(epochs, model, batch_size=batch_size, use_gpu=True):
    ...
    for epoch in range(epochs):
        with tqdm(total=int(650000/(256*batch_size)) * len(DS_train),bar_format='{desc} {n_fmt:>4s}/{total_fmt:<4s} {percentage:3.0f}%|{bar}| {postfix}',) as t:
            ...
            for inputs, labels in batch_norm_arrythdata_iter(DS_train, batch_size, use_random_noise=True, max_noise=5):
                if use_gpu:
        ...
        with tqdm(total=int(650000/(256*batch_size)) * len(DS_eval),bar_format='{desc} {n_fmt:>4s}/{total_fmt:<4s} {percentage:3.0f}%|{bar}| {postfix}',) as t:
            ...
            for inputs, labels in batch_norm_arrythdata_iter(DS_eval, batch_size, use_random_noise=True, max_noise=2):
                if use_gpu:
                    inputs, labels = inputs.cuda(), labels.cuda()
                optimizer.zero_grad()
                pre = model(inputs)
